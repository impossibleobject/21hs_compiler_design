...
module IR = struct
  ...
  (* An intermediate language with enough data types to represent
     meta-level environments as IL-level values.  Since a closure is
     pair consisting of an environment and a code pointer and the
     environment of a closure is a tuple of values representing the
     values of the context at the time the closure is created, we need
     to add N-ary tuples and projection to the IL.
