(* Example of writing a lexer by hand (part 0)
 *
 * The purpose of this file and handlex.ml is mainly to illustrate that
 * it is a bit annoying and tedious to have to write lexers by hand.
 *)
...
let rec lex (input:in_channel) : token list =
  ...
  let rec next (c:char) =
    ...
    else if  is_lparen c      then (emit LPAREN;
                                    next (input_char input))
    else if  is_rparen c      then (emit RPAREN;
				    next (input_char input))
    else if  is_character c   then lex_ident c
    else if  is_digit c       then lex_int c
    else if  is_star c        then (emit STAR;
                                    next (input_char input))
  ...
  and lex_ident c =
	...
	if (is_character c || is_digit c || is_underscore c)
	then
  ...
  and lex_int c =
	...
	then (Buffer.add_char buf c;
	      loop ())
	else (emit (Int (Int64.of_string (Buffer.contents buf)));
	      next c)
    in
      Buffer.add_char buf c;
      loop ()

  and exit () = List.rev (!acc)
...
let rec main () =
  try
    List.iter print_token (lex stdin);
  with
